...
app.get("/api/classify-number", async (req, res) => {
    ...
    const absNumber = Math.abs(number); // Ensure absolute value for calculations
    ...
    if (number < 0 && !isArmstrong(absNumber)) {
        ...
        try {
            ...
            return res.status(500).json({
                number: number,
                error: true,
                message: "Could not fetch fun fact. Please try again later."
            });
        }
    }
    ...
    // Return Response with Explicit 200 OK
    return res.status(200).json({
        ...
        properties: properties,
        digit_sum: digitSum, // Ensure valid digit sum
        fun_fact: funFact // Uses generic fact for negative numbers (except Armstrong)
    });
});
